...
exports.disablePA = asyncHandler(async(req,res)=>{
	...
	{
		res.status(200).json({success:false,data:"Production Area not available for disabling"})
	}
	else if(area.disabled === true)
	{
		res.status(200).json({success:false,data:"Production Area already disabled"})
	}
	else{
	let	updateRecord = {
		updatedAt: Date.now(),
	...
	await PA.findOneAndUpdate({_id:req.params.id},{
	$set:{ disabled:true },
	$push:{updateRecord:updateRecord}
	})
	res.status(200).json({success:true,data:"Production Area status changed"})
	}
})
...
exports.enablePA = asyncHandler(async(req,res)=>{
	...
	if(area.disabled === true)
	{
		...
		res.status(200).json({success:false,data:"Production Area already enabled"})
	}
})
